...
function getAccessibleName(el: Element): string {
  ...
  if (el instanceof HTMLButtonElement || el instanceof HTMLInputElement) {
    if ('value' in el && (el as HTMLInputElement).type !== 'password') {
...
function getAccessibleValue(el: Element): string {
  if (el instanceof HTMLInputElement) {
    if (el.type === 'password') return '';            // Privacy rule
...
const _elementToId = new WeakMap<Element, number>();

function getOrAssignId(el: Element): number {
...
function snapshotElement(el: Element, parentId: number): AccessNodeSnapshot {
  ...
  return {
    ...
    isDisabled:  (el as HTMLElement).getAttribute('aria-disabled') === 'true' ||
                 ('disabled' in el && (el as HTMLButtonElement).disabled),
    isHidden:    (el as HTMLElement).getAttribute('aria-hidden') === 'true',
    bounds: {
      x:      Math.round(rect.x),
      y:      Math.round(rect.y),
...
function buildTreeSnapshot(): AccessNodeSnapshot[] {
...
function send(msg: ContentToBackgroundMessage): void {
  try {
...
function onFocusIn(event: FocusEvent): void {
  ...
  const msg: FocusChangedMessage = { type: 'FOCUS_CHANGED', node };
  send(msg);
}
...
function scheduleSnapshot(): void {
  if (_snapshotTimer !== null) clearTimeout(_snapshotTimer);
  _snapshotTimer = setTimeout(() => {
    _snapshotTimer = null;
    const nodes = buildTreeSnapshot();
    if (nodes.length === 0) return;
    const msg: TreeSnapshotMessage = { type: 'TREE_SNAPSHOT', nodes };
    send(msg);
...
function init(): void {
  ...
  const loadMsg: PageLoadedMessage = {
    type:  'PAGE_LOADED',
    url:   safeUrl(),
